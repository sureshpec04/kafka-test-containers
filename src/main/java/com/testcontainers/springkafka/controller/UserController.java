package com.testcontainers.springkafka.controller;
...
public class UserController {
    ...
    public List<User> getUsers(@PathVariable(name = "firstName") String name) {
        List<com.testcontainers.springkafka.entity.User> users = userService.getUsers(name);
    ...
    public List<User> getAllUsers() {
        List<com.testcontainers.springkafka.entity.User> users = userService.findAllUsers();
        return users.stream().map(user -> new User(user.getId(), user.getFirstName(), user.getLastName())).collect(Collectors.toList());
